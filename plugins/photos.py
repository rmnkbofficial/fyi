...
Global = {"config": {}, "photos": []}

Global["config"]["path"] = "photos"
...
def preBuild(site):
	...
	if not os.path.isdir(page_path):
		logging.warning("No photo folder found at: %s", page_path)
	...
	for page in site.pages():
		...
		if page.path.startswith("%s/" % Global["config"]["path"]):
			...
			Global["photos"].append(context_post)


def preBuildPage(site, page, context, data):
	
	context['photos'] = Global["photos"]
	
	for post in Global["photos"]:
